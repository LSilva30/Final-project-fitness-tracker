...
export default function WorkoutList() {
  ...
  return (
    ...
    {workouts.map((exercise, i) => {     // mapping through the list of exercises, setting the data into single variable "exercise"
      return <li key={i}><Button variant="dark">done</Button> &nbsp; {`name: ${exercise.name}/ sets: ${exercise.sets}/ reps: ${exercise.reps}/`}</li>   // then returning it as a list dynamically by category: DISPLAYING IT TO SITE
    })}
  </ul>
  )
}
